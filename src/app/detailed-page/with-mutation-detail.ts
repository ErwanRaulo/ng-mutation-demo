...
export class WithMutationDetail {
  ...
  protected readonly createUser = mutation<string, User>({
    mutationFn: (name, abortSignal) => {
      ...
      abortSignal.addEventListener('abort', () => {
        if (!settled) {
          this.addLog(`createUser("${name || '(empty)'}") aborted: a newer call took over`);
        }
      });
      return fakeCreateUser(name, abortSignal).finally(() => {
        settled = true;
      });
    },
    onSuccess: (user) => {
  ...
  protected readonly deleteUser = mutation<User, void>({
    mutationFn: (user, abortSignal) => {
      ...
      abortSignal.addEventListener('abort', () => {
        if (!settled) {
          this.addLog(`deleteUser("${user.name}") aborted: a newer call took over`);
        }
      });
      return fakeDeleteUser(user, abortSignal).finally(() => {
        settled = true;
      });
    },
