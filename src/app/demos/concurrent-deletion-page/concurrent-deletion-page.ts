...
export class ConcurrentDeletionPage {
  ...
  protected deleteUserClick(user: User): void {
    ...
    this.deleteUser.mutate(user).then(
      () => {
        // Removed here, not in onSuccess: this .then() always reflects this specific
        // call's real outcome, even when it's superseded and onSuccess never fires.
        this.users.update((current) => current.filter((u) => u.id !== user.id));
      },
      () => {
        // a superseded call's rejection doesn't become unhandled.
      },
    );
  }
